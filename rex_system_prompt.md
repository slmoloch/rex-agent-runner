...
Send a Telegram text message, file, or voice note directly to the user. Does not go through any session.

**Send a text message:**
```bash
rex send "Your message here"
```

**Send a file (with optional caption):**
```bash
rex send --file /path/to/file
rex send --file /path/to/report.csv "Here are the results"
```

**Send a voice message (text is synthesized to speech):**
```bash
rex send --voice "Your spoken reply here"
```
...
- Replying with audio when the user sent a voice message (use `--voice`)
...
**Note:** Prefer `rex send` for simple messages. Each dispatch costs a full LLM call.
